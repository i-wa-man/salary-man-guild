...
Worker の結果に `knowledge_candidate: found: true` があれば、
**teams/{team}/CLAUDE.md に追記するかどうかを判断する。**

**判断基準（唯一のテスト）:**
> **「この知見がCLAUDE.mdに書いてあったら、同種の未来のタスクのアウトプットが変わるか？」**
> → Yes なら追記。No なら捨てる。

**追記ルール:**
1. `teams/{team}/CLAUDE.md` の末尾にある `## 蓄積された知見` セクションに追記
2. 既存の知見と重複していないか確認（重複なら追記しない）
3. 既存の知見を上書き/更新すべき場合は古い方を書き換える
4. 1件 = 1行。簡潔に書く（長文説明にしない）

**追記フォーマット:**
```markdown
## 蓄積された知見

- LP記事の導入文に具体的な数字を入れるとCVRが向上する傾向がある
- FastAPIではPydantic v2のmodel_validatorを使うとバリデーションコードが半減する
```

**肥大化防止:**
- 30件を超えたら、古い知見を見直して統合・削除する
- プロジェクト固有の知見は projects/{id}.yaml に書く（CLAUDE.mdには入れない）
- 「〜かもしれない」レベルの不確実な知見は入れない

**この判断にユーザー確認は不要。** 知識蓄積は業務判断ではなくシステム運用。
自律レベル（L1/L2/L3）のゲート対象外。

### 6. スキル化候補の確認

Worker の結果に `skill_candidate` があれば → 後述の「スキル化提案」を実行。

### 7. 全完了ならダッシュボード更新 → ユーザーに報告
