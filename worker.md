...
knowledge_candidate:
  ...
  insight: null             # 例: "LP記事では導入文に数字を入れるとCVRが上がる傾向"
  reason: null              # 例: "A/Bテスト結果から判明。今後のLP記事全般に適用可能"
...
**`knowledge_candidate` と `skill_candidate` は両方必須。書き忘れた報告は不完全とみなす。**

### 知見候補の判断基準

**「この知見を知っていたら、同種の未来のタスクのアウトプットが変わるか？」** で判断する。

| 基準 | found: true にする |
|------|-------------------|
| 知っていれば次の同種タスクの品質/効率が変わる | はい |
| 調査で判明した事実で、チームの専門知識に加えるべき | はい |
| 試行錯誤で得た「こうすると良い/悪い」の知見 | はい |
| 既に teams/{team}/CLAUDE.md に書いてある内容 | いいえ |
| このタスク固有の情報で汎用性がない | いいえ |
| プロジェクト固有の情報（→ projects/{id}.yaml に書く） | いいえ |
